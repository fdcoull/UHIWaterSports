...
if (isset($_GET['id']))
{
	if (isset($_SESSION['cart']))
	{
		...
		{
			$quantity = $_SESSION['cart'][$itemNo];
		}
		else
		{
			header("Location: cart?alert=cartedit4");
			exit();
		}
	}
	else
	{
		header("Location: cart?alert=cartedit3");
		exit();
	}
}
else
{
	header("Location: cart?alert=cartedit2");
	exit();
}
...
<body>
	...
	<div class="container-fluid">
		<div class="body-main">
			...
			<div class="row">
				<div class="col-sm-12">
					<h1>Edit quantity for item: <?PHP echo $itemNo; ?></h1>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-sm-6">
				<form method="POST" action="src/Cart/change_quantity">
					<table class="login-table">
						<tr>
							<td>Quantity</td>
							<td>
								<input type="number" name="quantity" value="<?PHP echo $quantity; ?>"/>
								<input type="hidden" name="item" value="<?PHP echo $itemNo; ?>"/>
							</td>
						</tr>
						<tr>
							<td><input class="btn btn-primary" type="submit" value="Change"></td>
						</tr>
					</table>
				</div>
			</div>
